...
function getRelevantItems(clientData: ClientData): RouteType[] {
  return clientData.isAdmin ? ROUTES.filter(r => (!!clientData.email && r.authenticated) || (!clientData.email && !r.authenticated)) :
    ROUTES.filter(r => (!!clientData.email && r.authenticated && r.userRights) || (!clientData.email && !r.authenticated && !r.userRights))
